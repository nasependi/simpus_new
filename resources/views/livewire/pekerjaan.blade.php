<div class="p-6">
    <flux:card class="shadow-lg rounded-lg">
        <div class="flex justify-between mb-4">
            <flux:heading size="xl">Data Pekerjaan</flux:heading>
            <div class="flex gap-4 items-center">
                <flux:input wire:model.live="search" placeholder="Cari pekerjaan..." icon="magnifying-glass" size="md" />
        ...
        <flux:table :paginate="$data">
            <flux:table.columns>
                ...
                <flux:table.column class="cursor-pointer" wire:click="sortBy('nama_pekerjaan')">
                    <div class="flex items-center">
                        <span>Nama Pekerjaan</span>
                        @if ($sortField === 'nama_pekerjaan')
                            <x-icon :name="$sortDirection === 'asc' ? 'arrow-up' : 'arrow-down'" class="w-3 h-3 text-muted-foreground ml-1" />
                        @endif
                    </div>
                </flux:table.column>
            ...
            <flux:table.rows>
                @foreach ($data as $item)
                    <flux:table.row>
                        ...
                        <flux:table.cell>{{ $item->nama_pekerjaan }}</flux:table.cell>
                        <flux:table.cell>
        ...
        {{-- Modal Tambah/Edit --}}
        <flux:modal name="pekerjaanModal" class="space-y-4 md:w-[50rem]">
            <flux:heading class="text-lg font-semibold">
                {{ $editId ? 'Edit' : 'Tambah' }} Pekerjaan
            </flux:heading>

            <flux:input wire:model="kode" label="Kode" required />
            <flux:input wire:model="nama_pekerjaan" label="Nama Pekerjaan" required />
        ...
        <flux:modal name="delete-pekerjaan" class="min-w-[22rem]">
            <div class="space-y-6">
                <div>
                    <flux:heading size="lg">Hapus Data Pekerjaan?</flux:heading>
