<div class="p-6">
    <flux:card class="shadow-lg rounded-lg">
        <div class="flex justify-between mb-4">
            <flux:heading size="xl">Detail Penjualan Obat</flux:heading>
        ...
        <flux:table :paginate="$data">
            <flux:table.columns>
                <flux:table.column wire:click="sortBy('penjualan_id')" class="cursor-pointer">No Faktur</flux:table.column>
            ...
            <flux:table.rows>
                ...
                <flux:table.row>
                    <flux:table.cell>{{ $item->penjualan->no_faktur ?? '-' }}</flux:table.cell>
        ...
        {{-- Modal Tambah/Edit --}}
        <flux:modal name="detailPenjualanModal" class="space-y-4 md:w-[50rem]">
            <flux:heading class="text-lg font-semibold">
                {{ $editId ? 'Edit' : 'Tambah' }} Detail Penjualan
            </flux:heading>

            <flux:select wire:model="penjualan_id" label="No Faktur" placeholder="Pilih Penjualan" required>
                @foreach($penjualanList as $id => $faktur)
        ...
        <flux:modal name="delete-detail-penjualan" class="min-w-[22rem]">
            <div class="space-y-6">
                <div>
