<div class="p-6">
    <flux:card class="shadow-lg rounded-lg">
        <div class="flex justify-between mb-4">
            <flux:heading size="xl">Status Pernikahan</flux:heading>
            <div class="flex gap-4 items-center">
                <flux:input wire:model.live="search" placeholder="Cari status..." icon="magnifying-glass" size="md" />
            ...
            <flux:table.rows>
                @foreach ($data as $item)
                    <flux:table.row>
                        ...
                        <flux:table.cell>{{ $item->status }}</flux:table.cell>
                        <flux:table.cell>
        ...
        {{-- Modal Tambah/Edit --}}
        <flux:modal name="statusPernikahanModal" class="space-y-4 md:w-[50rem]">
            <flux:heading class="text-lg font-semibold">
                {{ $editId ? 'Edit' : 'Tambah' }} Status Pernikahan
            </flux:heading>

            <flux:input wire:model="kode" label="Kode" required />
            <flux:input wire:model="status" label="Status Pernikahan" required />
        ...
        <flux:modal name="delete-status-pernikahan" class="min-w-[22rem]">
            <div class="space-y-6">
                <div>
                    <flux:heading size="lg">Hapus Data?</flux:heading>
                    <flux:text>Yakin ingin menghapus status ini? Data tidak dapat dikembalikan.</flux:text>
                </div>
                <div class="flex justify-end gap-2">
                    <flux:modal.close>
                        <flux:button variant="ghost">Batal</flux:button>
